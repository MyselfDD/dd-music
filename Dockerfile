...
RUN mvn -f /app/pom.xml clean package --spring.profiles.active=test
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/dd-music-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/dd-music-0.0.1.jar"]